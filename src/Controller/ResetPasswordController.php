...
class ResetPasswordController extends AbstractController
{
    ...
    public function request(Request $request, MailerInterface $mailer): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (!$limiter->consume()->isAccepted()) {
                $this->addFlash('danger', 'Trop de demandes de réinitialisation. Veuillez réessayer plus tard.');
    ...
    private function processSendingPasswordResetEmail(string $emailFormData, MailerInterface $mailer): RedirectResponse
    {
        ...
        $email = (new TemplatedEmail())
            ->from(new Address('admin@gamevault.sc7duyo5958.universe.wf', 'GameVault'))
            ->to((string) $user->getEmail())
            ->subject('Réinitialisation de votre mot de passe - GameVault')
